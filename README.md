...
#### Applications
- 	[Serverless App to automate my Job Search](https://github.com/SKnoedler/Portfolio/tree/master/Projects/Serverless%20Application): Finding the right job online can be tedious. Too many search results on job platforms make it difficult to find the right fit. In a short article (https://lnkd.in/d4BBqWE) I explain how you can build your own serverless application to automate your job search. Using serverless framwork, I built my own servelss application and used AWS Lambda to deploy it in AWS Cloud. The application crawls data from the job platform stepstone, extracts job titles and company names. I included an option to filter search results: Eliminate search results that include certain words in the job title, such as "Senior" positions. Filter company names and/ or specify words that must be included in the job title of your personal search results. The Application is scheduled with CloudWatch and stores data in a table in DynamoDB <br/> _Python, Serverless Framework (Node.js), AWS Lambda, CloudWatch, DynamoDB_
